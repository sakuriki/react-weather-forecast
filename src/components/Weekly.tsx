...
export const WeeklyWeather = ({ lat, lon, setToday }: DTO) => {
  ...
  useEffect(() => {
    WeatherService.getOneCall({ lat, lon })
      .then((response: any) => {
        setData(response.data);
        setToday(response.data.daily[0]);
        setIsLoading(false);
      })
      .catch((err: Error) => {
        console.log(err);
  ...
  return (
    <div className="py-5">
      <div className="flex flex-row gap-5">
        {!isLoading &&
          data.daily.slice(1).map((day: any, index: number) => {
            return (
              <div
                key={index}
